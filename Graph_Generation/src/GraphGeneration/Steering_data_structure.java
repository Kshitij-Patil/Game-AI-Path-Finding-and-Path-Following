...
//			  acceleration = PVector.mult(arrive_dir,-1*max_acceleration*(rad_of_dist-rad_of_sat)/rad_of_dist);
		  ...
		  float angle;
		  float rad_of_angle = 10;
		  float rad_of_satangle = 4;
		  
		  if(arrive_dir.y > 0 && arrive_dir.x > 0)
			  orientation = 360-rad_to_degree(java.lang.Math.atan(arrive_dir.y/arrive_dir.x));
		  else if(arrive_dir.x < 0 && arrive_dir.y < 0)
			  orientation = 180 - rad_to_degree(java.lang.Math.atan(arrive_dir.y/arrive_dir.x));
		  else if(arrive_dir.x > 0 && arrive_dir.y < 0)
			  orientation = -1 *rad_to_degree(java.lang.Math.atan(arrive_dir.y/arrive_dir.x));
		  else if(arrive_dir.x < 0 && arrive_dir.y > 0)
			  orientation = 180 + -1 *rad_to_degree(java.lang.Math.atan(arrive_dir.y/arrive_dir.x));
		  else 
			  orientation = rad_to_degree(java.lang.Math.atan(-arrive_dir.y/arrive_dir.x));
